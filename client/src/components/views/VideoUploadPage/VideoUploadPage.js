...
export default function VideoUploadPage(props) {
    ...
    const user = useSelector(state => state.user)       // redux store의 state.user를 반환
    ...
    const onDrop = (files) => {
        ...
        .then(response => {
            if(response.data.success) {
                ...
                let variable = {
                    url : response.data.url,    // video url
    ...
    const onSubmit = (e) => {
        ...
        .then(response => {
            if(response.data.success) {
                ...
                // 성공했다는 메시지를 띄우고 3초 이후에 LandingPage로 보낼 것.
        ...
        <div style={{ maxWidth: '700px', margin: '2rem auto'}}>
            <div style={{ textAlign: 'center', marginBottom : '2rem'}}>
                ...
                <Form onSubmit = {onSubmit}>
                    ...
                    {ThumbnailPath &&
                        <div>
                            <img src={`http://localhost:5000/${ThumbnailPath}`} alt="thumb" />
                        </div>
                    }
                    </div>
